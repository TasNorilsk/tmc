document.addEventListener('DOMContentLoaded', () => {
    ...
    // Clear button (add modal)
    let clearAddBtn = document.getElementById('clearAddBtn');
    if (!clearAddBtn) {
        const footer = catModalEl?.querySelector?.('.modal-footer');
        if (footer) {
            clearAddBtn = document.createElement('button');
            clearAddBtn.type = 'button';
            clearAddBtn.id = 'clearAddBtn';
            clearAddBtn.className = 'btn btn-sm btn-outline-secondary me-auto';
            clearAddBtn.textContent = 'Очистить';
            footer.prepend(clearAddBtn);
        }
    }
    ...
    function renderItemsGrouped(items) {
      ...
      `;
          ...
          <div class="tmc-col tmc-col--actions">
            <button type="button" class="btn btn-sm btn-outline-secondary meta" title="Изменить категорию/производителя">⚙</button>
            <button type="button" class="btn btn-sm btn-danger del"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash3" viewBox="0 0 16 16">
    ...
    async function openMetaModal({ mode, itemId = null, categoryId = null, manufacturerId = null }) {
        ...
        if (clearAddBtn) clearAddBtn.classList.toggle('d-none', !isAdd);
    ...
    function restoreLastCatManuInModal() {
        const lastCat = localStorage.getItem('lastCatId') || '';
        const lastManu = localStorage.getItem('lastManuId') || '';
        if (lastCat) catSelect.value = lastCat;
        if (lastManu) manufSelect2.value = lastManu;
        syncManufacturerUI();
    }
    ...
    itemsList.addEventListener('click', async (e) => {
        ...
        if (editable) {
            ...
            actions.insertAdjacentHTML(
                'beforeend',
                ' <button type="button" class="btn btn-sm btn-success save">✓</button> <button type="button" class="btn btn-sm btn-secondary cancel">✗</button>'
            );
            return;
        }
        ...
        if (saveBtn) {
            ...
            requestAnimationFrame(() => window.scrollTo({ top: __scrollY, left: 0, behavior: 'auto' }));
            return;
        }
    ...
    function renderRefs() {
        ...
        <td>
          ...
          <button type="button" class="btn btn-sm btn-danger ref-del" data-kind="cat">
      ...
      `;
        ...
        <td class="d-flex flex-wrap gap-2">
          ...
          <button type="button" class="btn btn-sm btn-danger ref-del" data-kind="manu">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash3" viewBox="0 0 16 16">
              <path d="M6.5 1h3a.5.5 0 0 1 .5.5v1H6v-1a.5.5 0 0 1 .5-.5M11 2.5v-1A1.5 1.5 0 0 0 9.5 0h-3A1.5 1.5 0 0 0 5 1.5v1H1.5a.5.5 0 0 0 0 1h.538l.853 10.66A2 2 0 0 0 4.885 16h6.23a2 2 0 0 0 1.994-1.84l.853-10.66h.538a.5.5 0 0 0 0-1zm1.958 1-.846 10.58a1 1 0 0 1-.997.92h-6.23a1 1 0 0 1-.997-.92L3.042 3.5zm-7.487 1a.5.5 0 0 1 .528.47l.5 8.5a.5.5 0 0 1-.998.06L5 5.03a.5.5 0 0 1 .47-.53Zm5.058 0a.5.5 0 0 1 .47.53l-.5 8.5a.5.5 0 1 1-.998-.06l.5-8.5a.5.5 0 0 1 .528-.47M8 4.5a.5.5 0 0 1 .5.5v8.5a.5.5 0 0 1-1 0V5a.5.5 0 0 1 .5-.5"/>
            </svg>
          </button>
